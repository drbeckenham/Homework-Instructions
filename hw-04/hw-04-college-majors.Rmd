...
The shape of the plot is a skewed bell, leaning to the left. There is one significant outlier to the right. The minimum amount is 22000 and the maximum is 110000. The standard deviatio is 11479.18, and the mean is 40151.45. The median is 36000. Quartile 1 iss 33000, which is the 25% range, while Quartile 3, which gives us the 75% range, is 45000. 
...
```
Engineering has the highest median income, so that would be my choice.
...
The plot looks at women and their majors who are recent grads. The Median Income for these majors is plotted against the number of women. Then there is a notation, via color, of those that are or are not stem fields. I wonder if there would be a difference if this had an addition of what the median income for men was in these fields , and they were plotted against the women.

```{r women-income-stem, eval = TRUE}
ggplot(data = college_recent_grads, aes(x = sharewomen, y = median, color = major_type)) +
  geom_point() +
  labs(
    x = "Proportion of Women in majors",
    y = "Median Income",
    color = "STEM major",
    title = "Median Income vs Proportion of Women in Major"
  )
```
...
I would love to see the median income of women across STEM fields as opposed to men across non-stem fields.

**Your analysis (code and visualization/summary statistics):**
```{r custom-analysis, eval = TRUE}
college_recent_grads %>%
  mutate(group = case_when(
    major_type == "stem" & sharewomen > 0.5 ~ "Women in STEM",
    major_type != "stem" & sharewomen <= 0.5 ~ "Men in Non-STEM"
  )) %>%
  filter(!is.na(group)) %>%
  group_by(group) %>%
  summarise(
...
First, I will admit I sought some help with this code. I wasn't sure how to get one against the other, so I got help from ChatGPT. In analysis, I found that, for me, I prefer the hard cold numbers, as opposed to any visualization. What I found here was shocking. Men in non-STEM majors were paid higher than women in STEM majors. STEM majors and jobs tend to be more specialized and require more training and study. This should equate to more money, but apprently it doesn't when it comes to women. This is very disconcerting. 
