...
You can find out more about the dataset by inspecting its documentation, which you can access by running `?edibnb` in the Console or using the Help menu in RStudio to search for `edibnb`. You can also find this information [henrow(re](https://rstudio-education.github.io/dsbox/reference/edibnb.html).
...
1.  How many observations (rows) does the dataset have? Instead of hard coding the number in your answer, use inline code.
    ...
    There are 13246 rows in the dataset.
...
2.  Run `View(edibnb)` in your Console to view the data in the data viewer. What does each row in the dataset represent?
    ...
    Each row represents an Airbnb in Edinborough. It includes a link, as well as things like the neighborhood and number of rooms.
...
*Which binwidth did you choose and why? I chose 100 because it was visually easier to see.*
...
*Which layout did you choose and why? I liked to one with nrow as it was a better way to visualize it side by side as opposed to difference boxes.*
...
edibnb %>%
  ...
  labs(
    x = "Price",
    y = "Neighbourhood",
    title = "Price Distributions for Top 5 Most Expensive Neighbourhoods"
  )
```
...
The neighburhod called New Town seems to have the highest typical prices.
B
-   Which has the most variation (spread) in prices?

Old Town seems to have the biggest variation in prices.

-   Are there any notable patterns or outliers?

New Town has both very high prices and high values. Looking at the visualization, the long tails would indicate outliers, which were extremely expensive prices.
...
-   How do Airbnb guests rate properties in general (are ratings generally high, low, or varied)? Ratings seem to be varied, but pretty centralized. There are some that do have outliers or more scores than others, but they are just that, outliers.
-   How do the neighborhoods compare to each other?

Everything is fairly even with the exclusion of Leith, which is probably outlier status.

-   Are there any neighborhoods that stand out as having particularly high or low ratings?
Leith is the highest, which is probably either a data coleciton error, or a total outlier, subject to some other external forces.
